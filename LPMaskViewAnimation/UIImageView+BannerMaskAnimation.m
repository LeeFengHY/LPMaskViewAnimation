//
//  UIImageView+BannerMaskAnimation.m
//  LPMaskViewAnimation
//
...
@implementation UIImageView (BannerMaskAnimation)
...
- (void)setFadeDuration:(NSTimeInterval)fadeDuration
{
    if (self.bannerImages.count >1) {
        ...
        self.fadeAnimationDuration = fadeDuration / (self.bannerImages.count + 3) / LPMultipled;
...
- (void)fadeBanner
{
    ...
    [self animationFadeWithComplete:^{
        if (!self.stop) {
            complete();
        }
    }];
...
{
    if (idx >= self.bannerImages.count) {
        idx = 0;
    }
    self.image = [UIImage imageNamed:self.bannerImages[idx]];
    [self reverseWithoutAnimation];
